...
public:
  ...
  Bool_t Overlaps(const Candidate *object) const 
  {
    ...
    if(fArray)
      {
	int ni=fArray->GetEntriesFast();
	for (int i=0; i< ni; i++ ) {
	  candidate= static_cast<Candidate *>(fArray->At(i));
    ...
    if(object->fArray)
      {
	int ni=object->fArray->GetEntriesFast();
	for (int i=0; i< ni; i++ ) {
	  candidate= static_cast<Candidate *>(object->fArray->At(i));
