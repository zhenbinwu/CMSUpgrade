...
//----------------------------------------------------------------------------
  if (process.find("TTBAR") != std::string::npos)
  {
    ...
    ANA = new DelAna(DEV);
    TString tempname = name.ReplaceAll(process.c_str(), "TTBAR_FL");
    MDelCut["TTFL"] = new DelCut(ANA, name.Data());
    tempname = name.ReplaceAll(process.c_str(), "TTBAR_SL");
    MDelCut["TTSL"] = new DelCut(ANA, name.Data());
    return 1;
  }
