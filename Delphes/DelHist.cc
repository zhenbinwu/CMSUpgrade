...
//----------------------------------------------------------------------------
  ...
  std::string Process = "BJJ_14TEV_HT1";
  ...
  boost::tokenizer<boost::char_separator<char> > tok(Outdir, sep);

  //for(boost::tokenizer<boost::char_separator<char> >::iterator tok_iter=tok.begin(), int i=0;
    //tok_iter!=tok.end(); tok_iter++, i++)
  //{
    //if (i == 0)  flag = *tok_iter;
    //if (i == 1)  PUCorJetEta = std::stati*tok_iter;
...
//----------------------------------------------------------------------------
  ...
  DP.InitDelPhes(Process, Pileup, "RawMET");
  DP.ReadDelPhes();
  DP.PreLooping();
  DP.Looping();
  DP.PostLooping();
