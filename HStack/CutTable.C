...
int main ( int argc, char *argv[] )
{
  ...
  Sample.push_back("TTBAR_13TEV_FLep");
  Sample.push_back("TTBAR_13TEV_SLep");
  Sample.push_back("WJETS_13TEV");
  Sample.push_back("ZJETS_13TEV");
...
int ElogTable(std::string dir, std::string pileup, std::vector<std::string> Sample)
{
  ...
  for(std::vector<std::string>::iterator Sit=Sample.begin();
      Sit!=Sample.end(); Sit++)
  {
    TString filename = dir+"/"+*Sit+"_"+pileup+".root";
    TFile *f = new TFile(filename.Data(), "R");
    if (!f->IsOpen()) return 0;
    //TFile f1("LoopCut_5_30/TTBAR_13TEV_NoPileUp.root");
    TH1F* c1 = (TH1F*)f->Get("CutFlow");
    ...
    for (int i = 1; i < c1->GetXaxis()->GetNbins(); ++i)
    {
      if (i > (order.size()+1)) break;
      char buf[100];
      sprintf(buf, "%.f(%f)",c1->GetBinContent(i) , c1->GetBinContent(i)/c1->GetBinContent(i-1)  );
  ...
  std::cout << std::endl;
  std::cout << "================================================================= " << std::endl;
  std::cout <<  "[b]" << "PUCorrection :" << dir <<  " PileUp : " << pileup  << "[/b]"<< std::endl;
